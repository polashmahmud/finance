<template>
  <q-page class="q-pa-md">
    <div class="row items-center justify-between q-mb-md">
      <div>
        <div class="text-h5 text-weight-bold">Notes</div>
        <div class="text-caption text-grey">{{ notes.notes.length }} notes</div>
      </div>
    ...
    <div v-if="pinnedNotes.length" class="q-mb-md">
      ...
      <div class="q-gutter-md">
        <q-card v-for="note in pinnedNotes" :key="note.id" class="finance-card" style="border-left: 4px solid #FFC107">
          <q-card-section>
            <div class="row items-start justify-between">
              <div class="col">
                ...
                <div class="text-caption text-grey q-mt-sm">
                  {{ note.createdAt }}
                  <q-badge v-if="note.transactionId" color="secondary" class="q-ml-sm">Linked</q-badge>
                </div>
              </div>
    ...
    <div class="swipe-hint q-mt-sm" v-if="unpinnedNotes.length">Swipe left to delete</div>

    <!-- Empty State -->
    <div v-if="!notes.notes.length" class="text-center text-grey q-mt-xl">
      <q-icon name="note" size="60px" class="q-mb-md" />
      <div class="text-h6">No notes yet</div>
    ...
    <q-dialog v-model="showAdd" position="bottom">
      <q-card style="width: 100%; max-width: 500px; border-radius: 16px 16px 0 0">
        <q-card-section class="row items-center q-pb-none">
          <div class="text-h6 text-weight-bold">New Note</div>
          <q-space />
          <q-btn flat round icon="close" v-close-popup />
        </q-card-section>
        <q-card-section>
          <q-form @submit.prevent="addNote" class="q-gutter-md">
            <q-input v-model="newNote.title" label="Title" outlined dense autofocus />
            <q-input v-model="newNote.description" label="Description" outlined type="textarea" rows="4" />
            <q-select
              ...
              map-options
            />
            <q-btn type="submit" unelevated rounded color="primary" label="Save Note" class="full-width" />
          </q-form>
        </q-card-section>
      </q-card>
    </q-dialog>
...
const txOptions = computed(() =>
  transactions.transactions.slice(0, 20).map((t) => ({
    label: `${t.type} - ${t.category || 'Transfer'} - ${t.amount}`,
    value: t.id,
  })),
)
