<template>
  <q-page class="q-pa-md q-pb-xl">
    ...
    <!-- Month Filter -->
    <div class="row q-gutter-sm q-mb-md" style="overflow-x: auto; flex-wrap: nowrap; padding-bottom: 4px">
      <q-btn :outline="selectedMonth !== 'all'" :flat="selectedMonth === 'all'" :color="selectedMonth === 'all' ? 'dark' : 'grey-7'" :text-color="selectedMonth === 'all' ? 'white' : undefined" dense rounded no-caps :label="$t('allTransactions.allTime')" @click="selectedMonth = 'all'" class="q-px-md" style="flex-shrink: 0" />
      <q-btn v-for="m in availableMonths" :key="m.value" :outline="selectedMonth !== m.value" :flat="selectedMonth === m.value" :color="selectedMonth === m.value ? 'dark' : 'grey-7'" :text-color="selectedMonth === m.value ? 'white' : undefined" dense rounded no-caps :label="m.label" @click="selectedMonth = m.value" class="q-px-md" style="flex-shrink: 0" />
    </div>

    <!-- Type Filter -->
    <div class="row q-gutter-sm q-mb-md">
      <q-chip v-for="f in typeFilters" :key="f.value" :selected="selectedType === f.value" :color="selectedType === f.value ? 'dark' : 'grey-3'" :text-color="selectedType === f.value ? 'white' : 'dark'" clickable @click="selectedType = f.value" dense>
        {{ f.label }}
      </q-chip>
    </div>
    ...
    <q-card class="finance-card" v-if="filteredTransactions.length">
      <q-list separator>
        <q-slide-item v-for="tx in filteredTransactions" :key="tx.id"
          ...
          <q-item class="touch-target">
            ...
            <q-item-section>
              <q-item-label class="text-weight-medium">{{ tx.category || $t('common.transfer') }}</q-item-label>
              <q-item-label caption>{{ tx.notes }} &middot; {{ tx.date }}</q-item-label>
            </q-item-section>
            <q-item-section side>
              <q-item-label :class="tx.type === 'income' ? 'amount-income' : tx.type === 'expense' ? 'amount-expense' : 'text-blue'" class="transaction-amount">
                {{ tx.type === 'income' ? '+' : tx.type === 'expense' ? '-' : '' }}{{ settings.currency }}{{ formatNumber(tx.amount) }}
              </q-item-label>
            </q-item-section>
          </q-item>
        </q-slide-item>
...
const availableMonths = computed(() => {
  ...
  return [...months]
    ...
    .map((m) => {
      ...
      const label = date.toLocaleDateString(locale.value === 'bn' ? 'bn-BD' : 'en-US', {
        year: 'numeric',
        month: 'long',
      })
