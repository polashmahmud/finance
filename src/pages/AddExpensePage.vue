    ...
    <q-card class="finance-card">
      <q-card-section>
        <q-form @submit.prevent="saveExpense" class="q-gutter-md">
          <!-- Amount -->
          <q-input v-model.number="form.amount" label="পরিমাণ" type="number" outlined :prefix="settings.currency"
            :rules="[val => val > 0 || 'সঠিক পরিমাণ লিখুন']" autofocus input-class="text-h5 text-weight-bold" />

          <!-- Category -->
          <q-select v-model="form.category" :options="categories.expenseCategories.map(c => c.name)" label="ক্যাটাগরি"
            outlined />


          <!-- Account -->
          <q-select v-model="form.accountId" :options="accountOptions" label="অ্যাকাউন্ট" outlined emit-value
            map-options />
          ...
          <!-- Submit -->
          <q-btn type="submit" unelevated rounded color="negative" label="ব্যয় সংরক্ষণ করুন" class="full-width"
            size="lg" icon="check" />
        </q-form>
      </q-card-section>
    </q-card>
